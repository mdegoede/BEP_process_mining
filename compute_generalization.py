...
for folder_path in maps:
    ...
    for file in files:
        ...
        gen_priv = generalization_evaluator.apply(original_log, privacy_pn, im, fm)
        gen_original = generalization_evaluator.apply(original_log, original_pn, im, fm)
...
df.to_csv('CalculateGED\conformance_results\gens_df.csv')

df = pd.read_csv('CalculateGED\conformance_results\gens_df.csv')
...
for df, df_name in zip(algos, df_names):
    for value in val:
        ...
        file_name = 'CalculateGED\conformance_results\gens_' + str(df_name) + '.csv'
        ...
        plt.savefig('CalculateGED\conformance_results\gens_' + str(df_name) + '_' + str(value) + '.png', bbox_inches='tight')
        plt.show()
