...
for folder_path in maps:
    ...
    for file in files:
        ...
        gen_priv = simplicity_evaluator.apply(privacy_pn)
        gen_original = simplicity_evaluator.apply(original_pn)
...
df.to_csv('CalculateGED\conformance_results\simp_df.csv')

df = pd.read_csv('CalculateGED\conformance_results\simp_df.csv')
...
for df, df_name in zip(algos, df_names):
    for value in val:
        ...
        file_name = 'CalculateGED\conformance_results\simp_' + str(df_name) + '.csv'
        ...
        plt.savefig('CalculateGED\conformance_results\simp_' + str(df_name) + '_' + str(value) + '.png', bbox_inches='tight')
        plt.show()
