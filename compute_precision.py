...
for folder_path in maps:
    ...
    for file in files:
        ...
        try:
            ...
            gen_priv = pm4py.precision_token_based_replay(original_log, privacy_pn, imp, fmp)
            gen_original = pm4py.precision_token_based_replay(original_log, original_pn, im, fm)

            print(gen_priv, gen_original, gen_original-gen_priv)
            gen_value = (gen_priv, gen_original, gen_original-gen_priv)
...
df.to_csv('CalculateGED\conformance_results\prec_df.csv')

#df = pd.read_csv('CalculateGED\conformance_results\prec_df.csv')
...
for df, df_name in zip(algos, df_names):
    for value in val:
        ...
        file_name = 'CalculateGED\conformance_results\prec_' + str(df_name) + '.csv'
        ...
        plt.savefig('CalculateGED\conformance_results\prec_' + str(df_name) + '_' + str(value) + '.png', bbox_inches='tight')
        plt.show()